...
3. Now, the format dialog will open. Refer [Format Data](/designer-guide/report-designer/compose-report/format-data/) section to set different formats to the data. Here, the `Currency` format is applied to the `Unit Price` dataset field.
...
2. In the properties view, enable the `Enable Link` option under `Link` category.
![Design report using textbox](/static/assets/on-premise/images/report-designer/report-items/textbox/enable-link-over-all-textbox.png)
3. Refer [Hyperlink](/designer-guide/report-designer/compose-report/link-data/#hyperlink) section and set the URL in the textbox.
