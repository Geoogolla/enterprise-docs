...
When the `Include Zero` property is enabled, the y-axis configures the range so that zero is included. But, whenever y values are big and differences among data points are hard to judge, setting `Include Zero` is disabled makes the y-axis set a range that makes the differences prominently visible.
