...
6. In the `Field Source` text box, enter the static value directly or set an [Expression](/on-premise/report-designer/compose-report/filter-data/#set-expression) for the field. Here, `=Month(Fields!OrderYear.Value)` expression is assigned to the `OrderMonth` field.
